...
```{r}
two_group_stats_template = data.frame(
  TestGroup=character(),
...
for (var in exp_vars)
{
  two_group_stats_template = 
    two_group_stats_template %>%
    mutate(!!as.name(var) := numeric())
}

two_group_stats = two_group_stats_template
```
...
```{r}
all_two_group_stats = two_group_stats_template

master_table = all_master_table
test_group = 'All'
  
for (test_var in all_variables)
{
...
two_group_stats %<>% add_row(!!!all_two_group_stats)
```
...
for (test_var in age_lt_90_variables)
{
  ...
  age_lt_90_two_group_stats %<>%
    add_row(!!!pval_list)
}

two_group_stats %<>% add_row(!!!age_lt_90_two_group_stats)
```


#### AMD Only 
```{r}
amd_only_two_group_stats = two_group_stats_template
...
two_group_stats %<>% add_row(!!!amd_only_two_group_stats)
```

#### AMD Only, Age < 90
```{r}
amd_only_age_lt_90_two_group_stats = two_group_stats_template
...
for (test_var in amd_only_variables)
{
  ...
  amd_only_age_lt_90_two_group_stats %<>%
    add_row(!!!pval_list)
}

two_group_stats %<>% add_row(!!!amd_only_age_lt_90_two_group_stats)
```
