...
## Prepare Abundance Tables {.tabset}
...
## Calculate Iga Index and ICI Score {.tabset}
...
## EDA on Indices and Scores {.tabset}
...
#### Log Boxplot: Distributions for Subjects
```{r}
ici_score %>%  
  select_if(is.numeric) %>% 
  mutate_all(list(~log(.))) %>%
  ...
  xlab('SubjectID') +
  ylab('Log ICI Score') +
  ggtitle('Log ICI Scores by Subject')
```
...
#### Log Boxplots: Distributions for Taxa
```{r, fig.height=12}
ici_score %>%
  select(count_filtered_subjects, short_glommed_taxa) %>%
...
index_table_names = c("iga_index", "ici_score", "alt_index")
...
master_table_names = c("iga_pos", "iga_neg", "iga_index", "ici_score", "alt_index")
