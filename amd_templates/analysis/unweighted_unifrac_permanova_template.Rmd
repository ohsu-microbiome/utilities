...
# MiSeq-${miseq_run_number} UNWEIGHTED UNIFRAC PERMANOVA {.tabset}
...
for (var in variables)
{
  ...
  heteroskedasticity_test = betadisper(unweighted_unifrac, sample_data %>% pull(var), type="median") %>% print()
  
  formula = 
    paste('unweighted_unifrac', '~', var) %>%
    as.formula()
...
formula = 
  paste('unweighted_unifrac', '~', paste(variables, collapse=' + ')) %>%
  as.formula()
...
for (var in amd_only_covariates)
{
  ...
  # unweighted_unifrac = as.matrix(unweighted_unifrac)
...
if ('UNWEIGHTED PERMANOVA' %in% sheets(wb))
{
  print(sprintf('removing worksheet %s', 'UNWEIGHTED PERMANOVA'))
  removeWorksheet(wb, sheet='UNWEIGHTED PERMANOVA')
}

print(sprintf('creating worksheet %s', 'UNWEIGHTED PERMANOVA'))
addWorksheet(wb, sheetName='UNWEIGHTED PERMANOVA')

openxlsx::writeData(wb, sheet='UNWEIGHTED PERMANOVA', x=all_results)
